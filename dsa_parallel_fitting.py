...
def compute_epidemic_fit_in_parallel(epidemic_obj_arr, start_idx, end_idx):
    ...
    for index in range(start_idx, end_idx):
        ...
        epidemic_obj.fit(laplace=laplace)
        res.append((index, epidemic_obj))
    comm.send(res, dest=0, tag=rank)


if __name__ == '__main__':
    laplace = sys.argv[1].lower() == 'true'
    epidemic_objects_array = pickle.load(open("epidemic_objects_array", "rb"))
    ...
    comm.Barrier()
    if laplace:
        print('Using Laplace posterior likelihood')
    ...
    if rank == 0:
        ...
        pickle.dump(epidemic_objects_array_fitted, open("epidemic_objects_array_fitted", "wb"), protocol=3)
